/*
    ...
    파일명은 크게 HEAD, NUMBER, TAIL의 세 부분으로 구성된다.
     ...
     2. NUMBER는 한 글자에서 최대 다섯 글자 사이의 연속된 숫자로 이루어져 있으며, 앞쪽에 0이 올 수 있다. 
...
const solution = (files) => {
    return files.sort((prev, next) => {
        prev = prev.match(/(\D*)(\d*)/i);
        next = next.match(/(\D*)(\d*)/i);
        
        const prevWords = String(prev[1].toLowerCase());
        const prevNumber = Number(prev[2]);
        const nextWords = String(next[1].toLowerCase());
        const nextNumber = Number(next[2]);

        // const prevWords = String(prev.toLowerCase().match(/\D+/i));
        // const prevNumber = Number(prev.match(/\d+/i));
        // const nextWords = String(next.toLowerCase().match(/\D+/i));
        // const nextNumber = Number(next.match(/\d+/i));

        if(prevWords > nextWords) return 1;
        if(prevWords < nextWords) return -1
        if(prevWords === nextWords) {
            if(prevNumber > nextNumber) return 1;
            if(prevNumber < nextNumber) return -1;
            if(prevNumber === nextNumber) return 0;
        };
    });
};
