...
const solution = (survey, choices) => {
  ...
  const typeObj = { R: 0, T: 0, C: 0, F: 0, J: 0, M: 0, A: 0, N: 0 };

  survey.forEach((item, idx) => {
    if (choices[idx] < 4) {
      typeObj[item[0]] += 4 - choices[idx];
    } else if (choices[idx] > 4) {
      typeObj[item[1]] += choices[idx] - 4;
    }
  });

  return typeArr.reduce((acc, curr, idx) => {
    if (idx % 2 === 0) {
      acc +=
        typeObj[curr] >= typeObj[typeArr[idx + 1]] ? curr : typeArr[idx + 1];
    }
    return acc;
  }, "");
};
